...
class PettiposRepository extends Disposable {
  CollectionReference _collection = Firestore.instance.collection(PetTipos.COLLECTION_NAME);
  ...
  Observable<List<PetTipos>> get getAll => Observable(_collection.snapshots().map((query) =>
  query.documents.map<PetTipos>((DocumentSnapshot document) => PetTipos.fromMap(document)).toList())
  );
